...
def parse_note_date(cell_value):
    """Robust parser for Excel Note Date in m/d/yyyy format or datetime object"""
    if not cell_value:
        return None
...
def insert_jsonl_into_same_sheet(input_dir, excel_file, reference_date_str):
    ...
    # Determine eligible rows based on Note Date (m/d/yyyy)
    eligible_rows = []
    for row in range(2, ws.max_row+1):
        date_val = parse_note_date(ws.cell(row=row, column=col_idx["Note Date"]).value)
        if date_val and date_val <= threshold_date:
            eligible_rows.append(row)

    if not eligible_rows:
        logging.warning("No eligible rows found prior to 45 days from reference date. Notes will not be inserted.")
        return
