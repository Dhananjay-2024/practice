...
def consolidate_excel_jsonl_insertion(
    ...
    The 'Note Date' of the new record is set to the best-matching target_date.
                    ...
                    with open(file_path, "r", encoding="utf-8-sig") as f:
                        for line in f:
                            ...
                            all_jsonl_records.append({
                                ...
                                "Note Date": None, # This will be assigned the target_date
                                "Note": rec.get("text", ""),
    ...
    # We now store the best_matching_target_date with the insertion point.
    
    # List of (priority_score, original_0_indexed_row_position, best_matching_target_date_for_new_note)
    scored_insertion_points = [] 
    ...
    for original_idx, row_data in enumerate(existing_rows_data):
        ...
        best_score_for_this_idx = float('inf') 
        best_target_date_for_this_idx = None
            ...
            for interval_idx, days_prior in enumerate(sorted_intervals):
                ...
                if diff_days >= 0: # row_note_date is on or after target_date
                    ...
                    if score < best_score_for_this_idx:
                        ...
                        best_target_date_for_this_idx = target_date
            
        # Add to our list of scored points.
        if best_target_date_for_this_idx: # Only add if a suitable target date was found for this spot
            scored_insertion_points.append((best_score_for_this_idx, original_idx, best_target_date_for_this_idx))
    ...
    if not scored_insertion_points: # Check if list is empty after sorting
    ...
    insertion_offsets = [0] * (len(existing_rows_data) + 1)
    ...
    for idx, rec in enumerate(all_jsonl_records):
        ...
        # Get the best available insertion point (lowest score, original index, and target date)
        score, original_insert_idx, assigned_note_date = scored_insertion_points.pop(0)
        ...
        logging.debug(f"Inserting record {idx+1} (score {score}) before original index {original_insert_idx} (current index {current_insert_idx}). New Note Date: {assigned_note_date}")

        # Copy Case from the row that will be immediately above the new insertion
        if current_insert_idx > 0:
            prev_row_data = combined_data[current_insert_idx - 1]
            rec["Case"] = prev_row_data[col_map["Case"] - 1]
            # rec["Note Date"] is *not* copied, it's assigned the 'assigned_note_date'
        ...
        # Assign the determined target date to the new record
